...
export class ZKChat {
  ...
  getUnreadCountDM = async (sender_pubkey: string, receiver: string, lastRead = 0) => {
    return this.DB.chats?.getUnreadCountDM(sender_pubkey, receiver, lastRead);
  };
  ...
  isEpochCurrent = async (epoch: string) => {
    ...
    const difference_in_seconds =
      Math.abs(serverTimestamp.getTime() - messageTimestamp.getTime()) / 1000;
  ...
  verifyRLNProof = async (proof: RLNFullProof) => {
    return RLN.verifyProof(vkey as any, proof);
  };

  verifySemaphoreProof = async (proof: SemaphoreFullProof) => {
    return Semaphore.verifyProof(semaphoreVkey as any, proof);
  };
  ...
  addChatMessage = async (chatMessage: ChatMessage) => {
    ...
    if (chatMessage.type === 'DIRECT') {
      data = {
        ...
        sender_address: chatMessage.sender.hash ? undefined : chatMessage.sender.address,
