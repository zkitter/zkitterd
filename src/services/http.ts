...
export default class HttpService extends GenericService {
    ...
    verifyAuth =
        ...
        async (req: Request, res: Response, next: NextFunction) => {
            ...
            if (signature) {
                ...
                const externalNullifier = await genExternalNullifier(
                    await getExternalNullifer(req)
                );
    ...
    handlePostChatMessage = async (req: Request, res: Response) => {
        const { messageId, type, timestamp, sender, receiver, ciphertext, rln, semaphore } =
            req.body;
        const signature = req.header('X-SIGNED-ADDRESS');
        const userDB = await this.call('db', 'getUsers');
    ...
    addRoutes() {
        ...
        this.app.post(
            '/v1/events/:clientId',
            jsonParser,
            this.wrapHandler(this.handleUpdateSSEClient)
        );
        this.app.get(
        ...
        this.app.get(
            ...
            this.wrapHandler(async (req, res) => {
                ...
                if (typeof req.query.link !== 'string') {
                    res.status(400).send(
                        makeResponse(`link must be present in query string.`, true)
                    );
                    return;
                }
                ...
                if (
                    model &&
                    model.updatedAt.getTime() + 1000 * 60 * 60 * 24 > new Date().getTime()
                ) {
        ...
        this.app.get(
            ...
            this.wrapHandler(async (req, res) => {
                ...
                const { oauth_token: token, oauth_token_secret: tokenSecret } =
                ...
                res.send(
                    makeResponse(`${TW_AUTH_URL}?${queryString.stringify({ oauth_token: token })}`)
                );
            })
        );
        ...
        this.app.post(
            ...
            this.wrapHandler(async (req, res) => {
                ...
                res.send(
                    makeResponse(`https://twitter.com/${auth.username}/status/${json.id_str}`)
                );
            })
        );
